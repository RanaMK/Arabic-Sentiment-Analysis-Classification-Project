...
#    random.shuffle( data )
    ...
    classifier.add(Dropout(dropout_prob[1]))
    classifier.add(Conv1D(filters=nb_filter, kernel_size=filter_sizes[0], padding='same', activation='relu'))
    classifier.add(MaxPooling1D(pool_size=2))
    classifier.add(Conv1D(filters=nb_filter, kernel_size=filter_sizes[1], padding='same', activation='relu'))
    classifier.add(MaxPooling1D(pool_size=2))
    classifier.add(Conv1D(filters=nb_filter, kernel_size=filter_sizes[2], padding='same', activation='relu'))
    classifier.add(MaxPooling1D(pool_size=2))
...
#    classifier.add(Dense(250, activation='relu'))
    ...
    else: classifier.add(Dense(num_classes, activation='softmax'))
...
#    # ==================================================
    ...
    else: classifier.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
...
#    else: model.compile(loss='categorical_crossentropy', optimizer='Adagrad', metrics=['accuracy'])
    ...
    checkpointer = ModelCheckpoint(filepath= 'C:\\Users\\paperspace\\Desktop\\Weights\\'+dataset_name+'_'+Round+'_'+Linked+'_'+model_variation+'_'+Vectors+'_weights_Ar_best.hdf5', verbose=1, save_best_only=False)
...
#    pd.DataFrame(model.fit(x=train_sequences, y=train_labels, batch_size=32, epochs=30, verbose=2).history).to_csv("C:\\Users\\paperspace\\Desktop\\Results\\history_"+dataset_name+".csv")
    ...
    classifier.fit(x=train_sequences, y=train_labels, batch_size=32, epochs=30, verbose=2)      
    ...
    # ==================================================
    print("Evaluate...")
    score, acc  = classifier.evaluate(test_sequences, test_labels,batch_size=32)
    ...
    model_feat = Model(inputs=classifier.input,outputs=classifier.get_layer('flatten_1').output)
    
    feat_train = model_feat.predict(train_sequences)
    print(feat_train.shape)
    
    feat_test = model_feat.predict(test_sequences)
...
#    classifier.fit(model_output, train_labels)

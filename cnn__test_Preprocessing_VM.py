...
datasets = list()
datasets = {
#        ('ASTD','C:\\Users\\paperspace\\Desktop\\data_csv_unbalanced\\ASTD-unbalanced-not-linked.csv',40),
#            ('ATT','C:\\Users\\paperspace\\Desktop\\data_csv_unbalanced\\ATT-unbalanced-not-linked.csv',568),
##            ('HTL','C:\\Users\\paperspace\\Desktop\\data_csv_unbalanced\\HTL-unbalanced-not-linked.csv',1110),
            ('MOV','C:\\Users\\paperspace\\Desktop\\data_csv_unbalanced\\MOV-unbalanced-not-linked.csv',2335)
            #,
#            ('PROD','C:\\Users\\paperspace\\Desktop\\data_csv_unbalanced\\PROD-unbalanced-not-linked.csv',234),
##            ('RES','C:\\Users\\paperspace\\Desktop\\data_csv_unbalanced\\RES-unbalanced-not-linked.csv',539),
#            ('LABR','C:\\Users\\paperspace\\Desktop\\data_csv_unbalanced\\LABR-unbalanced-not-linked.csv',882)        
        }
...
#    reviews = [ ( row["text"] , row["polarity"]  ) for row in csv.DictReader(open(file_name, encoding="utf8"), delimiter=',', quoting=csv.QUOTE_NONE) ]
    ...
    (body,rating)=LoadDataset_General.Load_Data(dataset_name)
    columns = {'body': body, 'rating': rating}
    data = pd.DataFrame(columns, columns = ['body', 'rating'])
    reviews = pd.DataFrame([[body, rating]])
    
    #Remove duplication
    data = pd.DataFrame.drop_duplicates(pd.DataFrame(reviews))
    ...
    checkpointer = ModelCheckpoint(filepath= 'C:\\Users\\paperspace\\Desktop\\Weights\\'+file_name+'_'+Round+'_'+Linked+'_'+model_variation+'_'+Vectors+'_weights_Ar_best.hdf5', verbose=1, save_best_only=False)
...
#        the_file.write(repr( le.classes_ ))
    ...
    pd.DataFrame(model.fit(x=train_sequences, y=train_labels, batch_size=32, epochs=2, verbose=2).history).to_csv("C:\\Users\\paperspace\\Desktop\\Results\\history_"+dataset_name+".csv")

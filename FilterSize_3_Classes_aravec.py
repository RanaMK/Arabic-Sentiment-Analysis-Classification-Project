...
#version2
embeddings = gensim.models.Word2Vec.load('C:\\Users\\paperspace\\Desktop\\Twt-CBOW\\v2\\tweet_cbow_300')
#embeddings = gensim.models.Word2Vec.load('E:\\data\\aravec\\v2\\Twt-CBOW')
...
#            data_test.iloc[i,0] = LoadDataset_General.Named_Entity_Recognition(data_test.iloc[i,0])
         file:///C:/Users/PAVILION G4/Desktop/Re%253a_LSTM_aravec_3_Classes/LSTM_3Classes_aravec_VM.py

#    random.shuffle( data )
    if dataset_name not in ('EG_NU','SemEval'):
        train_size = int(len(data) * val_split)
        train_texts = data.iloc[0:train_size,0].tolist()file:///C:/Users/PAVILION G4/Desktop/Re%253a_LSTM_aravec_3_Classes/LSTM_3Classes_aravec_VM.py
        ...
        tokenizer.fit_on_texts(train_texts)
        train_sequences = sequence.pad_sequences( tokenizer.texts_to_sequences( train_texts ) , maxlen=max_sent_len )
        test_sequences = sequence.pad_sequences( tokenizer.texts_to_sequfile:///C:/Users/PAVILION G4/Desktop/Re%253a_LSTM_aravec_3_Classes/LSTM_3Classes_aravec_VM.pyences( test_texts ) , maxlen=max_sent_len )
        train_matrix = tokenizer.texts_to_matrix( train_texts )
        test_matrix = tokenizer.texts_to_matrix( test_texts )
        embedding_weights = np.zeros( ( max_features , embeddings_dim ) )
        for word,index in tokenizer.word_index.items():

...
#    plot_model(model, to_file='C:\\Users\\paperspace\\Desktop\\Plots\\model_plot.png', show_shapes=True, show_layer_names=True)
    ...
    def create_model(model, learn_rate=0.01, momentum=0):        
        sgd = optimizers.SGD(learn_rate, momentum, nesterov=True)	
      
        # model compilation
        # ==================================================
        if num_classes == 2: model.compile(loss='binary_crossentropy', optimizer=sgd, metrics=['accuracy']) 
        else: model.compile(loss='categorical_crossentropy', optimizer=sgd, metrics=['accuracy'])
...
#    history = model.fit(x=train_sequences, y=train_labels, validation_data = (test_sequences, test_labels), batch_size=32, epochs=5, verbose=2)      
    ...
    plt.show()
    plt.savefig('E:\\train_test_acc_'+dataset_name+'.png')
    ...
    plt.show()
    plt.savefig('E:\\train_test_loss_'+dataset_name+'.png')

...
embeddings1 = dict( )
embeddings2 = dict( )
embeddings1 = gensim.models.KeyedVectors.load_word2vec_format("C:\\Users\\paperspace\\Desktop\\(CBOW58)-ASA-3B-CBOW-window5-3iter-d300-vecotrs.bin", binary=True,encoding='utf8', unicode_errors='ignore')
#embeddings2 = gensim.models.Word2Vec.load('C:\\Users\\paperspace\\Desktop\\Twt-CBOW\\Twt-CBOW')
#version 2
embeddings2 = gensim.models.Word2Vec.load('C:\\Users\\paperspace\\Desktop\\Twt-CBOW\\v2\\tweets_cbow_300')
...
#version2
#embeddings2 = gensim.models.Word2Vec.load('E:\\data\\aravec\\v2\\tweets_cbow_300')
...
#    random.shuffle( data )
    ...
    embedding1 = Embedding(max_features, embeddings_dim, input_length=max_sent_len, mask_zero=False, weights=[embedding_weights1] )(main_input)
    embedding2 = Embedding(max_features, embeddings_dim, input_length=max_sent_len, mask_zero=False, weights=[embedding_weights2] )(main_input)
    
    layer1 = []
    for embedding_layer in [embedding1, embedding2]:
        conv_pools = []
        for n_gram in filter_sizes:
            conv = Convolution1D(nb_filter=nb_filter, filter_length=n_gram, border_mode='valid', activation='relu', subsample_length=1, input_dim=embeddings_dim, input_length=max_sent_len)(embedding_layer)
    ...
    for layer in layer1: # no of embeddings used
        ...
        for (filter_size, input_feature_maps) in layer:
            conv = Convolution1D(nb_filter=nb_filter, filter_length=filter_size-1, border_mode='same', activation='relu')(input_feature_maps)
            pool = MaxPooling1D(pool_length=4, border_mode='same')(conv)
            last_layer.append(pool)
            
    Merged = Concatenate(axis=1)(last_layer)
    flat = Flatten()(Merged)
...
#    #Visualize the model in a graph
    plot_model(model, to_file='C:\\Users\\paperspace\\Desktop\\Results\\model_plot_Multi.png', show_shapes=True, show_layer_names=True)
...
#    checkpointer = ModelCheckpoint(filepath= 'E:\\Weights\\'+dataset_name+'_'+Round+'_'+Linked+'_'+model_variation+'_'+Vectors+'_weights_Ar_best.csv', verbose=1, save_best_only=False)
    ...
    with open('C:\\Users\\paperspace\\Desktop\\Results\\Results.txt', 'a') as the_file:
        ...
        the_file.write(str(len(body_all)))

...
#    early_stopping = EarlyStopping(patience=20, verbose=1)
    ...
    #without chepointer
    hist = model.fit(x=train_sequences, y=train_labels, batch_size=32, nb_epoch=5, validation_split=0.75, verbose=2, callbacks= [early_stopping])
...
#    pd.DataFrame(model.fit(x=train_sequences, y=train_labels, batch_size=32, epochs=30, verbose=2).history).to_csv("C:\\Users\\paperspace\\Desktop\\Results\\history_"+dataset_name+".csv")
    #with early stopping
    pd.DataFrame(hist = model.fit(x=train_sequences, y=train_labels, batch_size=32, nb_epoch=30, validation_split=0.75, verbose=2, callbacks= [early_stopping]).history).to_csv("C:\\Users\\paperspace\\Desktop\\Results\\history_"+dataset_name+".csv")

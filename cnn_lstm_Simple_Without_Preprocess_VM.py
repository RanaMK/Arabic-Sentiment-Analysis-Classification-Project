...
#    main_input = Input(shape=(max_sent_len,))
    ...
    model.add(Convolution1D(nb_filter=nb_filter, filter_length=filter_sizes[1], border_mode='valid', activation='relu', subsample_length=1, input_dim=embeddings_dim, input_length=max_sent_len))
    model.add(MaxPooling1D(pool_length=max_sent_len - filter_sizes[1] + 1))
    model.add(LSTM(100))
    model.add(Dense(1))
    if num_classes ==2:
...
#    print(model.summary())
    ...
    plot_model(model, to_file='E:\\model_plot_cnn_lstm_simple.png', show_shapes=True, show_layer_names=True)

...
#embeddings = gensim.models.Word2Vec.load('E:\\data\\aravec\\Twt-CBOW\\Twt-CBOW')
...
#    random.shuffle( data )
    ...
    for n_gram in filter_sizes:
        ...
        i+=1
    merged = merge([flat_name[0], flat_name[1], flat_name[2], flat_name[3]], mode='concat')    
    ...
    #2nd method
    plot_model(model, to_file='E:\\Results\\Plots\\model_plot.png', show_shapes=True, show_layer_names=True)
...
#    checkpointer = ModelCheckpoint(filepath= 'E:\\Weights\\'+dataset_name+'_'+Round+'_'+Linked+'_'+model_variation+'_'+Vectors+'_weights_Ar_best.hdf5', verbose=1, save_best_only=False)
#      
    #  model history
    # ==================================================
...
#    hist = model.fit(x=train_sequences, y=train_labels, batch_size=32, epochs=30, verbose=2)
    ...
    with open('E:\\Results\\Results.txt', 'a') as the_file:
...
#        the_file.write(repr( le.classes_ ))
    ...
    pd.DataFrame(model.fit(x=train_sequences, y=train_labels, batch_size=32, epochs=30, verbose=2).history).to_csv("E:\\Results\\history_"+dataset_name+".csv")
    
#    model.fit(x=train_sequences, y=train_labels, batch_size=32, epochs=1, verbose=2)      
